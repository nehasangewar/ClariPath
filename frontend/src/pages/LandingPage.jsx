    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: "'DM Sans', system-ui, sans-serif", overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes pageFlutter { 0%,100%{transform:rotateY(0deg)} 50%{transform:rotateY(-3deg)} }
        @keyframes featureIn { from{opacity:0;transform:translateX(32px) scale(0.94)} to{opacity:1;transform:translateX(0) scale(1)} }
        @keyframes openHint { 0%,100%{transform:translateY(0) scale(1);opacity:0.7} 50%{transform:translateY(-4px) scale(1.04);opacity:1} }
      ...
      <section ref={heroRef} style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', paddingTop:80 }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center', padding:'0 24px', maxWidth:900, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12, maxWidth:680, margin:'0 auto' }}>
            {stats.map((s) => (
              <div key={s.value} className="stat-card" style={{ borderRadius:16, padding:'20px 12px', textAlign:'center', background:C.white, border:`1px solid ${C.border}`, boxShadow:'0 2px 12px rgba(0,0,0,0.05)', transition:'all 0.3s' }}>
      ...
      <section id="features" style={{ padding:'100px 0 130px', background:C.bgAlt, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 32px' }}>
          ...
          {/* Book + Features layout — book slides left when open */}
          <div style={{ position:'relative', display:'flex', alignItems:'center', justifyContent:'center', minHeight:540, overflow:'hidden' }}>
            ...
            <div style={{
              ...
              transform: bookOpen ? 'translateX(-280px)' : 'translateX(0)',
              zIndex: 10,
            }}>
              <div className="book-scene" style={{ width:310, height:420 }}>
                <div className={`book-wrap ${bookOpen ? 'open' : ''}`} onClick={handleOpenBook}
                  style={{ width:280, height:380, margin:'20px auto 0', cursor:'pointer' }}>
                  ...
                  <div className="book-cover">
                    <div className="cover-inner">
                      ...
                      <div style={{ width:52, height:52, borderRadius:14, background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.18)', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:18, fontSize:24 }}>
                        📘
                      </div>

                      <h3 style={{ fontFamily:'Playfair Display, serif', fontSize:20, fontWeight:900, color:'rgba(255,255,255,0.93)', marginBottom:8, lineHeight:1.25, textAlign:'center' }}>
                        ClariPath<br />Features
                      </h3>
                      <p style={{ fontSize:11, color:'rgba(255,255,255,0.48)', marginBottom:24, textAlign:'center', lineHeight:1.55 }}>
                        4 intelligent features<br />built for your journey
                      </p>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:7, width:'100%', padding:'0 4px' }}>
                        {features.map((f, i) => (
                          <div key={i} style={{
                            ...
                            background: visibleFeatures.includes(i) ? 'rgba(255,255,255,0.13)' : 'rgba(255,255,255,0.04)',
                            border:`1px solid ${visibleFeatures.includes(i) ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.05)'}`,
                            transition:'all 0.45s ease',
                            display:'flex', alignItems:'center', gap:8,
                          }}>
                            <span style={{ fontSize:13 }}>{f.icon}</span>
                            <span style={{ fontSize:11, color: visibleFeatures.includes(i) ? 'rgba(255,255,255,0.82)' : 'rgba(255,255,255,0.25)', fontWeight:600, transition:'color 0.45s', flex:1 }}>{f.title}</span>
                            {visibleFeatures.includes(i) && (
                              <span style={{ width:6, height:6, borderRadius:'50%', background:'#fde68a', boxShadow:'0 0 6px #d97706', flexShrink:0 }} />
                            )}
                          </div>
                        ))}
                      </div>

                      <div className="open-pulse" style={{ marginTop:20 }}>
                        <div style={{ padding:'7px 16px', borderRadius:20, background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.18)', color:'rgba(255,255,255,0.78)', fontSize:11, fontWeight:600, display:'flex', alignItems:'center', gap:6 }}>
              ...
              {!bookOpen && (
                <p style={{ fontSize:13, color:C.textLight, textAlign:'center', maxWidth:200, lineHeight:1.55, marginTop:16 }}>
                  Click the book to reveal features ✨
                </p>
              )}
              {bookOpen && (
                <p style={{ fontSize:12, color:C.textLight, textAlign:'center', maxWidth:200, lineHeight:1.55, marginTop:12, opacity:0.7 }}>
            ...
            <div style={{
              ...
              transform: bookOpen ? 'translateX(20px)' : 'translateX(80px)',
              transition:'all 0.7s cubic-bezier(0.23,1,0.32,1)',
              pointerEvents: bookOpen ? 'all' : 'none',
              transitionDelay: bookOpen ? '0.3s' : '0s',
            }}>
              {features.map((f, i) => (
                <div
                  key={i}
                  className={`feat-card ${visibleFeatures.includes(i) ? 'shown' : ''}`}
                  style={{ transitionDelay: `${i * 0.05}s` }}
                  onClick={() => visibleFeatures.includes(i) && setActiveFeature(activeFeature === i ? null : i)}
                >
                  <div style={{
                    borderRadius:18, padding:'18px 20px', cursor: visibleFeatures.includes(i) ? 'pointer' : 'default',
                    background: activeFeature === i ? C.white : C.cardBg,
                    border: activeFeature === i ? `1.5px solid ${f.accent}35` : `1px solid ${C.border}`,
                    boxShadow: activeFeature === i ? `0 8px 32px rgba(0,0,0,0.08), 0 0 0 1px ${f.accent}10` : '0 2px 8px rgba(0,0,0,0.04)',
                    ...
                    {activeFeature === i && (
                      <div style={{ position:'absolute', top:-20, right:-20, width:100, height:100, borderRadius:'50%', background:`radial-gradient(circle, ${f.accent}08, transparent)`, filter:'blur(16px)' }} />
                    )}
                    <div style={{ display:'flex', alignItems:'flex-start', gap:14, position:'relative', zIndex:1 }}>
                      <div style={{
                        width:42, height:42, borderRadius:12, flexShrink:0,
                        background: activeFeature === i ? f.lightBg : `${f.accent}10`,
                        border:`1px solid ${activeFeature === i ? f.border : f.accent + '20'}`,
      ...
      <section style={{ padding:'80px 0 100px', background:C.bg, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 32px', position:'relative', zIndex:10 }}>
          ...
          <div style={{ textAlign:'center', marginBottom:56 }}>
            ...
            <h2 className="playfair" style={{ fontSize:38, fontWeight:900, color:C.text, letterSpacing:'-0.02em', marginBottom:12 }}>
              Built by students,<br />for students
            </h2>
            <p style={{ fontSize:15, color:C.textMid, maxWidth:420, margin:'0 auto', lineHeight:1.65 }}>
              Three engineering students who felt the confusion firsthand and decided to build the solution they wished existed.
            </p>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:24, maxWidth:860, margin:'0 auto' }}>
            {[
              {
                name:'Kabir Babarao Ubale',
                initials:'KU',
                role:'Frontend, UI/UX & AI',
                color:'#b45309',
                lightBg:'#fef3c7',
                linkedin:'https://www.linkedin.com/in/kabir-ubale-7a709b3b0',
                email:'kabirubale0358@gmail.com',
              },
              {
                name:'Neha Santosh Sangewar',
                initials:'NS',
                role:'Backend, System Architecture & AI',
                color:'#7c3aed',
                lightBg:'#f5f3ff',
                linkedin:'https://www.linkedin.com/in/neha-sangewar-84a452347',
                email:'nehasangewar14@gmail.com',
              },
              {
                name:'Priya Ganesh Akhadkar',
                initials:'PA',
                role:'Research & Documentation',
                color:'#047857',
                lightBg:'#ecfdf5',
                linkedin:'https://www.linkedin.com/in/priya-akhadkar-b051a6347',
                email:'priyaakhadkar31@gmail.com',
              },
            ].map((member, i) => (
              <div key={i} style={{
              ...
              >
                ...
                <div style={{
                  ...
                  background:`linear-gradient(135deg, ${member.color}22, ${member.color}40)`,
                  border:`2px solid ${member.color}30`,
                  display:'flex', alignItems:'center', justifyContent:'center',
                  marginBottom:18, fontSize:22, fontWeight:900,
                  fontFamily:'Playfair Display, serif', color:member.color,
                ...
                <div style={{ display:'inline-flex', alignItems:'center', padding:'3px 12px', borderRadius:20, background:member.lightBg, border:`1px solid ${member.color}25`, color:member.color, fontSize:11, fontWeight:600, marginBottom:22 }}>
                  {member.role}
                </div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%' }}>
                  ...
                  <a href={member.linkedin} target="_blank" rel="noopener noreferrer"
                    style={{
                      ...
                      borderRadius:12, background:C.bgAlt, border:`1px solid ${C.border}`,
                      textDecoration:'none', color:C.textMid, fontSize:13, fontWeight:500,
                  ...
                  >
                    {/* LinkedIn icon */}
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink:0, color:'#0077b5' }}>
                  ...
                  >
                    {/* Mail icon */}
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink:0, color:member.color }}>
